...
public class VoteListener implements Listener
{
    ...
    public void onVote(VotifierEvent event)
    {
        ...
        if (bmcPlayer == null)
        {
            plugin.getLogger().severe("投票プレイヤー読み込み不可能");
            return;
        }
        ...
        {
            Bukkit.dispatchCommand(bmcPlayer.getPlayer(), cmd.replace("%player", bmcPlayer.getName()) + " " + plugin.config
                    .getAdmin_pass()); // コマンド実行
        }
